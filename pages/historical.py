...
if not manifest:
    ...
    with st.container(border=True, key="tl_chart1_card"):
    ...
    if clicked_day is not None:
        ...
        # Without this, the marker highlight below lags a click behind --
        # fig1 (built above, before this click was known) would only pick
        # up the new selection on some *later* rerun, while the day-detail
        # panel (built after this point) already reads the fresh value.
        st.rerun()
    ...
    if tracked_slots:
        with st.container(border=True, key="tl_chart2_card"):
...
with st.expander("", icon=":material/settings:", expanded=not manifest):
    # ---------------------------------------------------------------------------
