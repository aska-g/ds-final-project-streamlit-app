"""Video inference for YOLO26s supervisorv1 PPE model.

Usage:
    python infer_video.py --source path/to/video.mp4
    python infer_video.py --source 0                      # webcam
    python infer_video.py --source video.mp4 --output out.mp4 --conf 0.25 --show

Defaults to runs/detect/yolo26s_supervisorv1_fixed_nomosaic_300e/weights/best.pt
— override with --weights or HIVIS_MODEL_PATH env var.
"""
import argparse
import os
import sys
...
DEFAULT_WEIGHTS = REPO_ROOT / "runs" / "detect" / "yolo26s_supervisorv1_fixed_nomosaic_300e" / "weights" / "best.pt"
...
def annotate_frame(frame, boxes, names):
    """Draw boxes + labels onto frame (in-place). Importable for Streamlit."""
...
def parse_args():
    p = argparse.ArgumentParser(description="YOLO26s supervisorv1 video inference")
    p.add_argument("--source", required=True, help="Video path, image path, or 0 for webcam (also supports RTSP/YouTube via opencv)")
    p.add_argument("--weights", default=None, help="Path to best.pt (default: runs/detect/yolo26s_supervisorv1_fixed_nomosaic_300e/weights/best.pt or $HIVIS_MODEL_PATH)")
...
def main():
    ...
    try:
        while True:
            ...
            if args.show:
                cv2.imshow("YOLO26s - video inference (q to quit)", frame)
